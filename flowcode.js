...
var setting = {
  ...
  lineheight: 20,
  loopAssistWidth: 50,
  canBeEndType: ",end,select-down,select-rightdown,loop-down,down,",
  //主要是防止，循环结构上方最后一个模块和循环连接起来
  canBeStartType: ",start,down,select-right,select-down,loop-up,loop-down,",
  canBeStart: function(obj) {
    return obj && setting.canBeStartType.indexOf(","+obj.type+",") >= 0 ? true: false;
  },
  canBeEnd: function(obj) {
    return obj && setting.canBeEndType.indexOf(","+obj.type+",")>=0 ? true: false;
  },
  ...
  getWH: function(type) {
    var width = setting.blockwidth;
    var height = setting.lineheight;
    switch (type) {
    case "start":
    case "end":
    case "loop":
    ...
    case "input":
    case "output":
      width = setting.blockwidth;
      height = setting.blockheight;
      break;
    default:
      if (setting.withLoopAssistWidth.indexOf(","+type + ",") >= 0) {
        width = setting.loopAssistWidth;
      }
    }
    return {
      "width": width,
...
var Common = {
  ...
  getCode: function(mode) {
    ...
    var code = '',i = 0,right = '',down = '',up = '',inputval = '',input;
  ...
  getPath:function(){
    ...
    alert("getPath未知类型！"+this.blockid);
  },
  /*判断是否是Line节点*/
  isLine: function() {
  ...
  insertBefore: function(block) {
    ...
    /*if (this.getPlace() === 0 && !setting.canBeStart(this)) {
      var line = new Line("down");
  ...
  insertAfter: function(block) {
    ...
    var down,next;
    /*if (this.getPlace() === this.parent.length - 1 && !setting.canBeEnd(this)) {
      down = new Line("down");
      down.insertAfter(this);
    }*/
    return this;
  },
  /*更改文字内容*/
  ...
  updateSize:function(){
    ...
    switch(this.type){
      case "select":
        ...
        this.allheight = h1 + setting.blockheight > h2 ? h1 + setting.blockheight: h2 + (setting.blockheight - setting.lineheight) / 2;
        break;
      case "loop":
        for(i = 0;i<this.up.length;i++){
        ...
        this.downwidth = w2;
        this.downheight = h2;
        this.allwidth = (w1 > w2 ? w1: w2) + setting.loopAssistWidth;
        this.allheight = h1 + h2 + setting.lineheight + setting.blockheight;
        this.leftwidth = (lw2 + setting.loopAssistWidth) > lw1?(lw2 + setting.loopAssistWidth):lw1;
        break;
      default:
  ...
  updatePosition:function(x,y,deep){
    ...
    switch(this.type){
      case "select":
        y = this.height;
        for(i = 0;i<this.down.length;i++){
          this.down[i].updatePosition(x,y,deep);
          y += this.down[i].allheight;
        }
        x = this.downwidth;
        y = (setting.blockheight - setting.lineheight)/2;
        for(i = 0;i<this.right.length;i++){
          this.right[i].updatePosition(x,y,deep);
      ...
      case "loop":
        y = - this.upheight;
        for(i = 0;i<this.up.length;i++){
          this.up[i].updatePosition(x,y,deep);
    ...
    if(this.x !== this._x || this.y !== this._y || deep){
  ...
  divUpdate: function(deep) {
    ...
    if(deep){
      this.divObject = $("[blockid="+this.blockid+"]");
      if(this.divObject.length === 0){
        this.divObject = null;
      }
    }
    if (!this.divObject) {
      ...
      if (temp.length > 0) {
        ...
        if(p){
          ...
          $("#" + flow.setting.viewId).append(this.divObject);
        }
        //节点鼠标点击对象时，自动选中对象
        if (this.isBlock()) {
  ...
  /*隐藏元素：是否更改内部分支*/
  hide: function(deep) {
    this.setVisible(false, deep);
  },
  show: function(deep) {
    this.setVisible(true, deep);
  },
  delete: function() {
    if (this.divObject) {
    ...
    if(this.parent && $.isArray(this.parent)){
      ...
      delete flow.blocks[this.blockid];
      for(var i in this.assists){
...
function Line(type, attrs) {
  ...
  if (type.indexOf("assist") >= 0) {
    ...
    flow.assists[this.blockid] = this;
  }
  else
  {
...
Line.prototype.updateAssist = function() {
  //console.log("updateFoot:%s %o",this.blockid,this);
    ...
    if (this.parent && this.type.indexOf("assist") >= 0) {
      ...
      case "select-assist-line1":
        this.x = 0;
        this.y = 0 + parent.downheight + setting.blockheight - setting.lineheight;
        this.height = parent.allheight - parent.downheight - setting.blockheight;
        break;
      case "select-assist-line2":
        this.x = 0 + parent.downwidth;
        this.y = parent.right[0].y + parent.rightheight - setting.lineheight;
        this.height = parent.allheight - parent.rightheight - (setting.blockheight - setting.lineheight) / 2;
        break;
      case "select-assist-line3":
        this.x = 0 + setting.blockwidth;
        this.y = 0 + (setting.blockheight - setting.lineheight) / 2;
        this.width = parent.downwidth - setting.blockwidth;
        break;
      case "select-assist-line4":
        this.x = 0 + setting.blockwidth;
        this.y = 0 + parent.allheight - setting.lineheight;
        this.width = parent.downwidth - setting.blockwidth;
        break;
      case "loop-assist-line1":
        this.x = 0 - parent.leftwidth;
        this.y = 0 + (setting.blockheight + setting.lineheight) / 2 + parent.upheight;
        this.height = parent.downheight + setting.blockheight - setting.lineheight;
        break;
      case "loop-assist-line2":
        this.x = 0 + parent.allwidth - setting.loopAssistWidth;
        this.y = 0 + setting.lineheight;
        this.height = parent.allheight - 3 * setting.lineheight;
        break;
      case "loop-assist-line3":
        this.x = 0 + setting.blockwidth;
        this.y = 0;
        this.width = parent.allwidth - setting.blockwidth - setting.loopAssistWidth;
        break;
      case "loop-assist-line4":
        this.x = 0 + setting.blockwidth;
        this.y = 0 + parent.downheight + setting.blockheight - setting.lineheight + parent.upheight;
        this.width = parent.allwidth - setting.blockwidth - setting.loopAssistWidth;
        break;
      case "loop-assist-line5":
        this.x = 0 - parent.leftwidth;
      ...
      case "loop-assist-line6":
        this.x = 0 - parent.leftwidth;
        this.y = 0 + parent.downheight + setting.blockheight + parent.upheight;
        break;
      case "loop-assist-line7":
        this.x = 0 + parent.allwidth - setting.loopAssistWidth;
        this.y = 0 ;
      ...
      case "loop-assist-line9":
        ...
        break;
      case "loop-assist-line10":
        this.x = 0 - parent.leftwidth + setting.loopAssistWidth;
        this.y = 0 + (setting.blockheight - setting.lineheight) / 2+ parent.upheight;
        this.width = parent.leftwidth - setting.loopAssistWidth ;
        break;
      case "loop-assist-line11":
        this.x = 0 - parent.leftwidth + setting.loopAssistWidth;
        this.y = 0 + parent.downheight + setting.blockheight+ parent.upheight;
...
function Block(type, attrs) {
    ...
    var i,temp = (
    this.blockid = "flow_block_" + (++Block.count),
    //当前blockid
    this.type = type,
    this.text = '',
    ...
    this.upheight = 0);
    if(type === "select"){
            //alert("addline 函数调整，需要更改参数");
        this.right=[];
        this.right.push(new Line("select-right",{parent: this.right}));
        this.down = [];
        this.down.push(new Line("select-down", {parent: this.down }));
        var rightdown = new Line("select-rightdown",{ parent: this.down });
    ...
    if (setting.getAssistNumber(this.type) > 0) {
        var num = setting.getAssistNumber(this.type);
        this.assists = [];
        for (i = 0; i < num; i++) {
...
var flow = {
    ...
    log:function(){
        ...
        try {
            if(arguments.length === 1){
                ...
                console.log(arguments);/*!需要完善*/
            }
            if(flow.setting.trace){
                console.trace();
            }
        }catch(err){
        }
    ...
    /*自动建立对象*/
    initData: function(str) {
        /*已经初始化过对象了*/
        if(flow.data.root && flow.data.root.length > 0){
            var bak = [];
            for(var i = 0;i<flow.data.root.length;i++){
                bak.push(flow.data.root[i]);
            }
            flow.deleteAll(true);
            flow.action.reinit(bak,newStr);
        }else{
            flow.action.init(str);
        }
        if (typeof str === "string") {
            str = $.parseJSON(str);
        }
        flow.data.root = flow.addObj(str);
        flow.view.update();
        if(flow.starts.length > 0){
            flow.view.changeChoosed(flow.starts[0]);
        }
        return flow;
    },
    ...
    add:function(key,attr){
        ...
        if(typeof key === "string"){
            if(flow.setting.block.indexOf(key)>=0){
                return flow.addBlock(key,attr);
            }
            if(flow.setting.line.indexOf(key)>=0 || key.indexOf("line")>=0){
    ...
    */
    update: function(deep) {
        ...
        for(i = 0;i<root.length;i++){
            if($.isArray(root[i]) && root[i].length > 0){
                ...
                y = root[i][0].y + vy_;
                for(j = 0;j<root[i].length;j++){
                    root[i][j].updatePosition(x,y,deep);
                ...
                root[i].updatePosition(x,y,deep);
                y += root[i].allheight;
            }
        }
        var array = flow.view._needUpdateArray;
    ...
    dropBlockTo: function(block, div) {
        if (block.attr("blockid")) {
            /**移动block元素到div位置*/
            block = flow.blocks[block.attr("blockid")];
            var newPs = flow.blocks[$(div).attr("blockid")];
            if(!newPs){
                alert("拖动到未知元素上了");
                flow.log("拖动到未知元素上了,%o",div);
                return;
            }
            if(block.type === "start" || block.type === "end" || block.type === "note"){
                alert("该节点不允许拖动到其他节点上！");
                return;
            }
            ...
            newPs = block.getPath();
            flow.action.changePlace(data,newPs,oldPlace);
            flow.view.update();
        } else {
            ...
            block.insertAfter(flow.blocks[div.attr("blockid")]);
            flow.view.update();
            flow.view.changeChoosed(block);
            flow.action.addBlock(block.getData(),block.getPath());
        }
    },
    ...
    changeTextEvent:function(block,text){
        ...
        flow.action.changeText(block.getPath(),block.text,text);
        block.text = text;
        flow.fire("textchange");
    },
    /**当停止拖动元素*/
    onStopDrag:function(e,n){
        ...
        if(flow.view._latestDrop === "view"){
            ...
            if(e.srcElement.innerText !== e.target.innerText){
                //防止拖动超出范围
                return true;
            }
            var viewPs = $("#"+flow.setting.viewId).position();
            var newPs = {"x":e.pageX - viewPs.left - e.offsetX,"y":e.pageY - viewPs.top - e.offsetY};
            if(target.attr("blockid")){
                //移动生成后的元素
                block = flow.blocks[target.attr("blockid")];
                if(!block){
                    return;
                }
                flow.action.changePosition(block.getPath(),newPs,{"x":block.x,"y":block.y});
                block.updatePosition(newPs.x,newPs.y);
            }else{
            var blocks = [];
            block = flow.addBlock(type,{x:e.pageX - viewPs.left - e.offsetX,y:e.pageY - viewPs.top - e.offsetY});
            blocks.push(block);
            block.parent = blocks;
            if(type === "start"){
            ...
            flow.data.root.push(blocks);
            flow.action.addBlock(flow.getData(blocks),(flow.data.root.length - 1)+"");
            }
            flow.view.update();
        }
        /*flow.view.update();
    ...
    insertBlockByPath:function(block,path){
        ...
        for(var i = 0;i<paths.length -1;i++){
            ...
            if(!now){
                alert("添加节点错误！"+path);
                flow.log("添加节点错误！%o : %s",block,path);
            }
        }
        if(now){
            block.parent = now;
    ...
    deleteChoosed:function(){
        ...
        flow.action.delete(block.getData(),block.getPath());
        block.delete();
        flow.view.update();
    },
    /**更改当前选中的元素*/
    changeChoosed: function(obj) {
        ...
        if(obj.isBlock){
            ...
            block = flow.blocks[obj.attr("blockid")];
        }
        flow.log("选中元素");
        flow.log(obj);
        flow.log(block);
...
flow.action = {
    /*初始化图像*/
    init:function(str){
        flow.history.add("init",str);
    },
    init_redo:function(obj){
        flow.deleteAll();
        flow.data.root = flow.addObj(obj);
        flow.view.update();
    },
    ...
    reinit:function(bak,newStr){
        flow.history.add("reinit",{'bak':bak,'new':newStr});
    },
    reinit_redo:function(obj){
        flow.deleteAll();
        flow.data.root = flow.addObj(obj.new);
        flow.view.update(true);
    },
    reinit_undo:function(obj){
        flow.deleteAll();
        flow.data.root = obj.bak;
        flow.view.update(true);
    },
    deleteAll:function(){
        var bak = flow.getData();
        flow.deleteAll();
        flow.history.add("deleteAll",bak);
    },
    deleteAll_redo:function(bak){
        flow.deleteAll();
    },
    deleteAll_undo:function(bak){
        flow.data.root = flow.addObj(bak);
        flow.view.update(true);
    },
    addBlock:function(data,path){
        flow.history.add("addBlock",{'data':data,'path':path});
    },
    ...
    delete:function(data,path){
        flow.history.add("delete",{'data':data,'path':path});
    },
    ...
    changePlace:function(data,path,bak){
        flow.history.add('changePlace',{"data":data,"path":path,"bak":bak});
    },
    ...
    changePosition:function(path,newPs,bak){
        flow.history.add("changePosition",{"path":path,"new":newPs,"bak":bak});
    },
    ...
    changeText:function(path,oldText,newText){
        flow.history.add("changeText",{"path":path,"new":newText,"bak":oldText});
    },
...
flow.history = {
    ...
    add: function(name, key) {
        ...
        var actions = history.actionlist;
        actions.splice(history.flag,actions.length - history.flag);//删除多余的动作
        if(name !== "changeText"){
            actions.push({
                "name": name,
                "key": key
        ...
        history.flag += 1;
        console.log("add history: "+name+" %o",key);
    },
    redo: function() {
        if(flow.history.flag >= flow.history.actionlist.length){
            return false;
        }
        var action = flow.history.actionlist[flow.history.flag];
        if(!action){
            return false;
        }
        if(action.name && flow.action[action.name+"_redo"]){
            flow.action[action.name+"_redo"](action.key);
        }
        flow.history.flag += 1;
    },
    undo: function() {
        if(flow.history.flag <= 0){
            return false;
        }
        flow.history.flag -= 1;
        var action = flow.history.actionlist[flow.history.flag];
        ...
        if(action.name && flow.action[action.name+"_redo"]){
            flow.action[action.name+"_undo"](action.key);
        }
    },
    canRedo:function(){
        if(flow.history.flag >= flow.history.actionlist.length){
...
error.addError = function(msg,obj){
     error._error_.push({"msg":msg,"obj":obj});
     if(obj){
        flow.view.changeChoosed(obj);
     }
     alert(msg);
